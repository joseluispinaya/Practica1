...
def consultar_urlscan(url):
    ...
    if not uuid:
        return f"ERROR AL ENVIAR (status={status_code})", None
    ...
    if err is not None:
        return f"NO REPORTADO ({err})", uuid
    ...
    if malicious:
        return f"Malicioso (score={score})", uuid

    if score:
        return f"No claramente malicioso (score={score})", uuid

    return f"NO REPORTADO", uuid
...
def generar_excel(url, gsb, vt, us, uuid):
    wb = Workbook()
    ws = wb.active
    ws.title = "Resultados"
    ...
    ws.append([
        url, gsb, vt, us, uuid, fecha
    ])

    output_path = "resulscanMejorado.xlsx"
    wb.save(output_path)

    print(f"\n Archivo Excel generado correctamente: {output_path}\n")
...
def analizar_url(url):
    ...
    generar_excel(url, gsb, vt, us, uuid)
